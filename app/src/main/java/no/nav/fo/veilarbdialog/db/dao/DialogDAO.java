...
public class DialogDAO {
    ...
    public List<DialogData> hentDialogerForAktorId(String aktorId) {
        return database.query(SELECT_DIALOG + "WHERE aktor_id = ?",
                this::mapTilDialog,
                aktorId
        );
    }
    ...
    public DialogData hentDialog(long dialogId) {
        return database.queryForObject(SELECT_DIALOG + "WHERE dialog_id = ?",
                this::mapTilDialog,
                dialogId
        );
    }
    ...
    private DialogData mapTilDialog(ResultSet rs) throws SQLException {
        long dialogId = rs.getLong("DIALOG.dialog_id");
        return DialogData.builder()
                .id(dialogId)
                .aktorId(rs.getString("DIALOG.aktor_id"))
    ...
    private List<HenvendelseData> hentHenvendelser(long dialogId) {
        return database.query("SELECT * FROM HENVENDELSE h LEFT JOIN DIALOG d ON h.dialog_id = d.dialog_id WHERE dialog_id = ?",
