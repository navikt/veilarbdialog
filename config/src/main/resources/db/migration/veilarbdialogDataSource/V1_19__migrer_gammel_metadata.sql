...
SET ELDSTE_ULESTE_FOR_BRUKER = (SELECT min(h.SENDT)
                                FROM HENVENDELSE h
                                WHERE h.SENDT > d.LEST_AV_BRUKER_TID AND h.DIALOG_ID = d.DIALOG_ID);
UPDATE DIALOG d
SET ELDSTE_ULESTE_FOR_VEILEDER = (SELECT min(h.SENDT)
                                  FROM HENVENDELSE h
                                  WHERE h.SENDT > d.LEST_AV_VEILEDER_TID AND h.DIALOG_ID = d.DIALOG_ID);
...
SET VENTER_PA_NAV_SIDEN = (SELECT min(h.SENDT)
                           ...
                           WHERE d.DIALOG_ID = h.DIALOG_ID AND h.SENDT > d.SISTE_FERDIGBEHANDLET_TID AND
                                 h.AVSENDER_TYPE = 'BRUKER') AND d.VENTER_PA_NAV_SIDEN IS NULL;
