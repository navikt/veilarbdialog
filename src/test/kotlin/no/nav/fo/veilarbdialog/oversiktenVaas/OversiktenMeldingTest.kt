...
class OversiktenMeldingTest {
    ...
    fun `Melding for utgått varsel skal ha riktige verdier`() {
        ...
        val melding = OversiktenMelding.forUtgattVarsel(fnr = fnr, operasjon = OversiktenMelding.Operasjon.START, LocalDateTime.now(), erProd = false)
...
//        assertThat(melding.hendelse.dato).isCloseTo(LocalDateTime.now(), within(100, ChronoUnit.MILLIS)) // TODO: test tidspunkt
    }

    @Test
    fun `Melding for utgått varsel skal ha riktig URL for prod`() {
        val fnr = "1234567891234"
        val melding = OversiktenMelding.forUtgattVarsel(fnr = fnr, operasjon = OversiktenMelding.Operasjon.START, LocalDateTime.now().plusDays(10), erProd = true)
    ...
    fun `Stoppmelding for utgått varsel skal ha riktig URL for prod`() {
        val fnr = "1234567891234"
        val melding = OversiktenMelding.forUtgattVarsel(fnr = fnr, operasjon = OversiktenMelding.Operasjon.STOPP, LocalDateTime.now().plusDays(10), erProd = true)
        assertThat(melding.hendelse.lenke).isEqualTo("https://veilarbpersonflate.intern.nav.no/aktivitetsplan")
