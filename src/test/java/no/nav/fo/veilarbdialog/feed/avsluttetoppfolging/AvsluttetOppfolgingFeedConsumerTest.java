...
public class AvsluttetOppfolgingFeedConsumerTest {
    ...
    public void skalIkkeOppdatereSisteIdHvisException() {
        doThrow(new RuntimeException("Mock exception")).when(dialogDataService).settDialogerTilHistoriske(null, null);
        List<AvsluttetOppfolgingFeedDTO> elements = asList(feedElement(new Date(), null, null));

        try {
            new AvsluttetOppfolgingFeedConsumer(dialogDataService, dao).lesAvsluttetOppfolgingFeed(null, elements);
        } finally {
    ...
    public void skalAvslutteForAlleElementerIFeed() {
        ...
        verify(dialogDataService).settDialogerTilHistoriske(aktor1, date1);
        verify(dialogDataService).settDialogerTilHistoriske(aktor2, date2);
